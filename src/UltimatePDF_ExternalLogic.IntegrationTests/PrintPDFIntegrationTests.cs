...
public class PrintPDFIntegrationTests : IClassFixture<BrowserPoolResetFixture> {

    private readonly HelloWorldWebFixture web;
    private readonly MockRestReceiverFixture rest;

    public PrintPDFIntegrationTests(
    ...
    public void PrintPDF_ToS3_HappyPath_UploadsPdfToPresignedUrl() {
        ...
        var s3Endpoints = new S3Endpoints {
            PdfPreSignedUrl = $"{rest.BaseUrl}/s3/object",
        };
        ...
        Assert.Single(rest.StoredS3Objects);
        var pdf = rest.StoredS3Objects[0];
        Assert.Equal((byte)'%', pdf[0]);
        Assert.Equal((byte)'P', pdf[1]);
        Assert.Equal((byte)'D', pdf[2]);
        Assert.Equal((byte)'F', pdf[3]);
