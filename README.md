...
Alternatively, setting the `RestCaller` field `SendBinariesAsBase64` to `True` makes the external logic POST the Base64 encoding of the PDF (and of the logs ZIP) as a `text/plain` body instead of a binary body. In that mode the receiving REST methods take the body as Text and decode it with `Base64Decode` before persisting it. Use it when an edge layer in front of your application rejects the binary POST — see [Known limitations](#known-limitations).
...
1. Call the Server Action `PrintToPDF_Advanced_ToRest`, fill the RestCaller parameter
    ...
    * SendBinariesAsBase64 - Optional, defaults to `False`. When `True`, both uploads send the Base64 encoding of the bytes as a `text/plain` body instead of a raw binary body. The matching template endpoints are `StorePath = /rest/pdf/StoreBase64` and `LogPath = /rest/pdf/StoreLogsBase64`, which decode the text before persisting it. Base64 grows the request body by roughly 33%, so check the encoded size against the receiving application's own request-size limits.
...
* Some edge layers in front of an ODC application (a WAF, an API gateway) reject the REST Store POST with a 403 because its body and content type are binary. The rejection shows up in the logs as an error on the `Store` call, usually carrying an HTML error page as the response body. Workaround: set the `RestCaller` field `SendBinariesAsBase64` to `True`, which sends the PDF and the logs ZIP as a Base64 `text/plain` body instead, and point `StorePath` / `LogPath` at REST methods that `Base64Decode` the body (`StoreBase64` / `StoreLogsBase64` in the template). Note that Base64 grows the body by roughly 33%.
